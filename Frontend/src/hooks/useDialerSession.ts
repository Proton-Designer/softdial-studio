...
export function useDialerSession(initialSessionId: string | null = null) {
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      const batchContacts: BatchContactPreview[] = (batch?.contacts ?? []).map((c: { id: string; name: string; phone: string }) => ({
        id: c.id,
        name: c.name,
        phone: c.phone,
      }));
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      channel.on('broadcast', { event: '*' }, ({ event, payload }: { event: string; payload: Record<string, unknown> }) => {
