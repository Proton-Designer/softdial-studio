...
export function ImportContactsModal({ open, onClose, onSuccess, campaignId }: ImportContactsModalProps) {
  ...
  const isFieldDisabled = (field: string, currentIndex: number) => isFieldMappedElsewhere(field, currentIndex);
  ...
  const pendingCount = columns.filter((c) => c.status === 'incomplete' || !c.mapped_field || c.mapped_field === '').length;

  return createPortal(
    <div className="fixed inset-0 flex items-center justify-center p-2" style={{ zIndex: 10000 }} role="dialog" aria-modal="true">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div
                ...
                className="space-y-4"
              >
                <p className="text-[#B0BEC5]">Upload a .csv file. We'll parse it and help you map columns.</p>
                {error && (
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-2xl p-12 text-center transition-colors ${
                    dragOver ? 'border-[#00D9FF] bg-[#00D9FF]/5' : 'border-white/20 hover:border-white/30'
                  }`}
                >
                  {loading ? (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Select value={importCategory} onValueChange={setImportCategory}>
                    ...
                    <SelectContent className="bg-[#1E2A3A] border-white/10 max-h-[14rem] [&_[data-radix-select-viewport]]:!max-h-[12.5rem]" style={{ zIndex: 10001 }}>
                      {IMPORT_CATEGORIES.map((cat) => (
                        <SelectItem key={cat} value={cat} className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white cursor-pointer">
                          {cat}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-[#1E2A3A] border-b border-white/10">
                        <th className="p-3 text-sm font-medium text-[#B0BEC5]">Column header in file</th>
                    ...
                    <tbody>
                      {columns.map((col, index) => (
                        <tr key={col.original_header} className="border-b border-white/5 hover:bg-white/5">
                                  ...
                                  return (
                                    <SelectItem
                                      ...
                                      className={cn(
                                        ...
                                        isMappedElsewhere && 'text-[#78909C] opacity-70 cursor-not-allowed'
                                      )}
                                    >
                                      {opt}
                                    </SelectItem>
