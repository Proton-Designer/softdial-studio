...
export function DialerIdle({
  ...
  const canDial = userNumbers.length > 0 && selectedFromNumber && fullPhoneNumber.length >= 10 && !isDialing;
  ...
  return (
    ...
    >
        ...
        <div className="mb-3 w-full" ref={pickerRef}>
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 text-[#B0BEC5] transition-transform ${numberPickerOpen ? 'rotate-180' : ''}`} />
            </button>
            {numberPickerOpen && (
      ...
      {error && (
        <p className="mt-3 text-sm text-[#FF3D00] text-center">{error}</p>
      )}

      {isDialing && (
        <p className="mt-2 text-sm text-[#00D9FF]">Connecting...</p>
      )}
    </motion.div>
  );
}
