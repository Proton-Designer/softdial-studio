...
export function CampaignManageModal({
  ...
  useEffect(() => {
    if (addExistingOpen) {
      listContacts().then(setContactsForPicker).catch(() => setContactsForPicker([]));
    }
  }, [addExistingOpen]);
    ...
    try {
      const agentCallControlId =
        typeof window !== 'undefined'
          ? (window.localStorage.getItem('softdial_active_agent_call_control_id') ?? '').trim() || undefined
          : undefined;
      ...
      console.warn('[CampaignManageModal] start parallel dial failed', { campaignId, error: message });
      toast.error(message);
    } finally {
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => handleUpdateStatus(v as 'active' | 'paused' | 'completed')}
                  >
                    ...
                    <SelectContent className="bg-[#1E2A3A] border-white/10" style={{ zIndex: 10001 }}>
                      <SelectItem value="active" className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">Active</SelectItem>
                      <SelectItem value="paused" className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">Paused</SelectItem>
                      <SelectItem value="completed" className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">Completed</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <button
              ...
              <div className="bg-[#1E2A3A]/60 rounded-2xl border border-white/5 p-6">
                ...
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  ...
                  <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                    ...
                    <SelectContent className="bg-[#1E2A3A] border-white/10" style={{ zIndex: 10001 }}>
                      {CATEGORIES.map((c) => (
                        <SelectItem key={c} value={c} className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">
                          {c}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <Select value={contactStatusFilter} onValueChange={setContactStatusFilter}>
                    ...
                    <SelectContent className="bg-[#1E2A3A] border-white/10" style={{ zIndex: 10001 }}>
                      {CONTACT_STATUSES.map((s) => (
                        <SelectItem key={s} value={s} className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">
                          {s}
                        </SelectItem>
                      ))}
                    </SelectContent>
                        ...
                        <tbody>
                          {paginatedLeads.map((lead) => (
                            ...
                            >
                              ...
                              <td className="py-3 pr-2">
                                <span
                                  className={`px-2 py-0.5 rounded text-xs ${lead.status === 'completed'
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setLeadsPage((p) => Math.min(totalPages - 1, p + 1))
                            }
                            className="px-3 py-1 rounded bg-[#1E2A3A] disabled:opacity-50 text-white text-sm"
                          >
                            Next
      ...
      <AnimatePresence>
        {startDialModalOpen && (
          <motion.div
            ...
            style={{ zIndex: 10000 }}
          >
            <div className="absolute inset-0 bg-black/60" onClick={() => !startingDial && setStartDialModalOpen(false)} aria-hidden />
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              ...
              <p className="text-sm text-[#B0BEC5] mb-4">
                We&apos;ll dial up to <span className="text-white font-semibold">{linesCount}</span> numbers at once and connect you instantly when someone answers.
              </p>
              <p className="text-sm text-[#B0BEC5] mb-4">
                Campaign: <span className="text-white">{campaign?.name ?? 'Campaign'}</span>
              </p>
              {userNumbers.length === 0 ? (
                <p className="text-sm text-[#FFB300] mb-5">Purchase a phone number in Settings to call from.</p>
              ) : (
                <div className="mb-5">
              ...
              <Select value={String(linesCount)} onValueChange={(v) => setLinesCount(Number(v) as 1 | 2 | 3 | 4 | 5)}>
                <SelectTrigger className="w-[120px] bg-[#1E2A3A] border-white/10 text-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#1E2A3A] border-white/10" style={{ zIndex: 10001 }}>
                  {[1, 2, 3, 4, 5].map((n) => (
                    <SelectItem key={n} value={String(n)} className="bg-[#1E2A3A] hover:bg-white/10 focus:bg-white/10 text-white focus:text-white">
                      {n}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <div className="mb-5">
                ...
                <p className="text-xs text-[#B0BEC5]/70 mt-1">
                  We&apos;ll call this number when a human answers. Leave empty if you are already connected via WebRTC/SIP.
                </p>
              </div>
          ...
          >
            ...
            >
              <p className="text-white mb-6">
                An active dialer session already exists for this campaign. Open the Dialer to resume it, or stop it and start a new session.
              </p>
      ...
      <AnimatePresence>
        {
          (confirmRemoveContactId !== null || confirmBulkRemove) && (
            <motion.div
              ...
              style={{ zIndex: 10000 }}
            >
              <div className="absolute inset-0 bg-black/60" onClick={() => { setConfirmRemoveContactId(null); setConfirmBulkRemove(false); }} aria-hidden />
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
              ...
              >
                ...
                <div className="flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => { setConfirmRemoveContactId(null); setConfirmBulkRemove(false); }}
                    className="px-4 py-2 rounded-lg bg-[#1E2A3A] text-white hover:bg-[#252F3E]"
                  >
                    Cancel
                  </button>
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 10000 }}>
      ...
      >
        ...
        <div className="p-4 overflow-y-auto flex-1">
          {available.length === 0 ? (
            <p className="text-[#B0BEC5]">No contacts available to add (all are already in this campaign).</p>
          ) : (
