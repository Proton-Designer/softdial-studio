...
import { Search, Upload, Plus, MoreVertical, Phone, Mail, Building2, MapPin, Loader2, Trash2, ChevronLeft, ChevronRight, Globe } from 'lucide-react';
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {paginatedContacts.map((contact, index) => {
            const name = contactDisplayName(contact);
            const initials = name.replace('—', '').split(' ').map(n => n[0]).filter(Boolean).join('').slice(0, 2) || '?';
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 min-w-0 md:min-w-0 md:[grid-template-columns:repeat(4,minmax(0,1fr))]">
                    <div className="min-w-0 overflow-hidden">
                      <div className="font-bold text-white text-lg mb-1 line-clamp-2 min-w-0 break-words" title={name}>{name}</div>
                      <div className="flex items-center gap-2 text-sm text-[#B0BEC5] min-w-0 overflow-hidden">
                        <Building2 className="w-4 h-4 flex-shrink-0" />
                        <span className="line-clamp-2 min-w-0 break-words" title={businessType}>{businessType}</span>
                      </div>
                      <div className="text-sm text-[#B0BEC5] flex items-center gap-1 mt-0.5 flex-wrap overflow-hidden">
                        <span className="px-2 py-0.5 rounded-full text-xs bg-white/10 truncate max-w-full">{category}</span>
                        <span className="px-2 py-0.5 rounded-full text-xs bg-white/10 truncate max-w-full">{contactStatus}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 min-w-0 overflow-hidden">
                      {email && (
                        <div className="flex items-center gap-2 text-sm text-[#B0BEC5] min-w-0 overflow-hidden">
                          ...
                          <span className="truncate min-w-0 block" title={email}>{email}</span>
                        </div>
                      )}
                      {website && (
                        <div className="flex items-center gap-2 text-sm text-[#B0BEC5] min-w-0 overflow-hidden">
                          <Globe className="w-4 h-4 text-[#00D9FF] flex-shrink-0" />
                          <span className="truncate min-w-0 block" title={website}>{website}</span>
                        </div>
                      )}
                      <div className="flex items-center gap-2 text-sm text-[#B0BEC5] min-w-0 overflow-hidden">
                        <Phone className="w-4 h-4 text-[#00D9FF] flex-shrink-0" />
                        <span className="truncate min-w-0 block" title={phone}>{phone}</span>
                      </div>
                    </div>
                    ...
                    <div className="min-w-0 overflow-hidden">
                      <div className="flex items-center gap-2 text-sm text-[#B0BEC5] mb-2 min-w-0 overflow-hidden">
                        ...
                        <span className="truncate min-w-0 block" title={contact.address ?? undefined}>{contact.address ?? '—'}</span>
                      </div>
                      <div className="text-xs text-[#B0BEC5] truncate min-w-0 block" title={`Added ${new Date(contact.created_at).toLocaleDateString()}`}>
                    ...
                    <div className="flex items-center justify-end gap-2 min-w-0 overflow-hidden flex-shrink-0">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-[#1E2A3A] border-white/10 text-white">
                          <DropdownMenuItem
                            variant="destructive"
                            onClick={(e) => handleDeleteOne(e as unknown as React.MouseEvent, contact.id)}
                            disabled={deleting}
      ...
      {!loading && filteredContacts.length > CONTACTS_PER_PAGE && (
        <div className="flex items-center justify-between gap-4 mt-6 py-4 px-2 border-t border-white/5">
          <span className="text-sm text-[#B0BEC5]">
            Showing {pageStart + 1}–{Math.min(pageStart + CONTACTS_PER_PAGE, filteredContacts.length)} of {filteredContacts.length}
          </span>
