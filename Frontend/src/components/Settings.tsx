...
import { User, Phone, CreditCard, Users as UsersIcon, Zap, Shield, Bell, Globe, Trash2, Search, Loader2 } from 'lucide-react';
...
export function Settings() {
  const { user } = useAuth();
  const { numbers: phoneNumbers, loading: phoneNumbersLoading, error: phoneNumbersError, refetch: refetchPhoneNumbers } = usePhoneNumbers();
  ...
  const [searchResults, setSearchResults] = useState<{ phone_number: string; monthly_cost?: string }[]>([]);
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {[
                        { label: 'Email notifications', sublabel: 'Get campaign reports daily', state: emailNotifications, setState: setEmailNotifications },
                        { label: 'SMS alerts', sublabel: 'Missed calls & voicemails', state: smsAlerts, setState: setSmsAlerts },
                        { label: 'Browser push', sublabel: 'Real-time call notifications', state: browserPush, setState: setBrowserPush },
                      ].map((pref) => (
                        <label key={pref.label} className="flex items-center justify-between cursor-pointer p-4 bg-[#1E2A3A]/60 rounded-xl hover:bg-[#1E2A3A] transition-colors">
                          <div>
          ...
          {activeTab === 'phone' && (
            ...
            >
              ...
              <div className="space-y-4 mb-8">
                {phoneNumbersLoading ? (
                  ...
                  <div className="bg-[#1A2332]/60 backdrop-blur-xl rounded-2xl p-8 border border-white/5 text-center">
                    <p className="text-[#B0BEC5]">No phone numbers yet. Search and purchase below.</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="text-2xl font-bold text-white mb-1">{phone.phone_number}</div>
              ...
              <div className="bg-gradient-to-r from-[#0066FF]/10 to-[#00D9FF]/10 backdrop-blur-xl rounded-2xl p-8 border border-[#0066FF]/20">
                ...
                <p className="text-[#B0BEC5] mb-4">Search by area code (e.g. 415, 920) or leave blank to browse US numbers.</p>
                ...
                {searchLoading && (
                  <div className="py-4 text-center text-[#B0BEC5]">Searching Telnyx marketplace…</div>
                )}
                {searchAttempted && !searchLoading && searchResults.length === 0 && (
                  <div className="py-4 text-center text-[#B0BEC5] text-sm">
                    No numbers found for this area. Try a different area code or leave blank for US numbers.
                  </div>
                )}
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {searchResults.map((n) => (
                      ...
                      >
                        ...
                        <span className="text-[#B0BEC5] text-sm">{n.monthly_cost ? `$${n.monthly_cost}/mo` : ''}</span>
                        <button
                        ...
                        >
                          {purchaseLoading === n.phone_number ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                          Purchase
                        </button>
                      </div>
                    ))}
              ...
              <div className="bg-[#1A2332]/60 backdrop-blur-xl rounded-2xl p-8 border border-[#00D9FF]/20 shadow-[0_4px_24px_rgba(0,102,255,0.15)] mb-8">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    ...
                    <div className="text-4xl font-bold text-white mt-4">$149<span className="text-lg text-[#B0BEC5]">/month</span></div>
                  </div>
                  <button className="px-6 py-3 bg-gradient-to-r from-[#00D9FF] to-[#0066FF] rounded-xl text-white font-semibold shadow-[0_4px_24px_rgba(0,217,255,0.3)] hover:shadow-[0_6px_32px_rgba(0,217,255,0.4)] transition-all">
                    Upgrade Plan
                  </button>
                ...
                <div className="space-y-3 mb-6">
                  {['5,000 minutes included', '10 team seats', 'Advanced analytics', 'Local presence', 'Priority support'].map((feature, i) => (
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="h-2 bg-[#1E2A3A] rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-[#00D9FF] to-[#0066FF] rounded-full" style={{ width: '65%' }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <div className="h-2 bg-[#1E2A3A] rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-[#00E676] to-[#00D9FF] rounded-full" style={{ width: '80%' }} />
                    </div>
                  </div>
                </div>
              </div>
