...
For parallel dialing, **use the Application ID**, not the SIP connection ID. Telnyx's `POST /calls` API expects a value in the `connection_id` field; their docs state it is *"The ID of the Call Control App (formerly ID of the connection)"*. So:
...
- Set:
  ...
  - **Webhook URL:**  
    ...
    (e.g. `https://lahordgefwsiqzvbfpyp.supabase.co/functions/v1/telnyx-webhook`)
